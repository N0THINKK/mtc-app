...
namespace mtc_app.features.admin.presentation.screens
{
    public class MasterDataEditorForm : Form
    {
        ...
        {
            this.Size = new Size(500, 680); // Tinggi sedikit ditambah untuk ruang password
            ...
            {
                Dock = DockStyle.Fill, FlowDirection = FlowDirection.TopDown, WrapContents = false, Padding = new Padding(24, 20, 24, 20), AutoScroll = true
            };
        ...
        private void GenerateDynamicFields()
        {
            if (_category == "User")
            {
                ...
                if (_isEditMode) 
                {
                    AddPasswordInput("Password Lama (Wajib diisi jika ingin ganti password)", "old_password");
                    AddPasswordInput("Password Baru (Kosongkan jika tidak diganti)", "new_password");
                }
                else 
                {
                    AddPasswordInput("Password (Default: 123456 jika dibiarkan kosong)", "new_password");
                }
            }
        ...
        private void AddInput(string labelText, string fieldKey, string value, bool isNumeric = false)
        {
            ...
            _inputControls.Add(fieldKey, txtInput); 
        }

        // Fungsi khusus untuk input Password (Karakter disembunyikan)
        private void AddPasswordInput(string labelText, string fieldKey)
        {
            ...
            { 
                ...
                UseSystemPasswordChar = true // Menyembunyikan teks jadi bintang/titik
            };
            pnlContainer.Controls.Add(txtInput);
            pnlForm.Controls.Add(pnlContainer);
            _inputControls.Add(fieldKey, txtInput); 
        ...
        private void AddComboBox(string labelText, string fieldKey, string[] options, string selectedValue)
        {
            ...
            pnlForm.Controls.Add(pnlContainer);
            _inputControls.Add(fieldKey, cmbInput); 
        }
        ...
        {
            if (_isEditMode && _rowData.ContainsKey(key) && _rowData[key] != null) return _rowData[key].ToString();
            return "";
        }
        ...
        private async void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            // Ekstraksi teks dari form
            foreach (var input in _inputControls)
            {
                string val = input.Value is ComboBox cmb ? cmb.Text : input.Value.Text;
                
                // Pengecualian khusus: Kolom password boleh dibiarkan kosong
                if (input.Key == "old_password" || input.Key == "new_password")
                {
            ...
            // Validasi Klien: Pastikan ngisi password lama kalau mau ganti password baru
            if (_isEditMode && _category == "User")
            {
                ...
                {
                    MessageBox.Show("Untuk mengubah password, Anda WAJIB memasukkan Password Lama!", "Peringatan Keamanan", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
            }
            ...
            try
            {
                ...
                if (success) {
                    ...
                    MessageBox.Show("Penyimpanan gagal, data tidak tersimpan.", "Gagal", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            catch (Exception ex)
            {
                // Pesan error dari `throw new Exception` di Dapper akan ditangkap di sini!
                MessageBox.Show(ex.Message, "Gagal Disimpan", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            finally
            {
