...
namespace mtc_auto_export
{
    ...
    class Program
    {
        ...
        private static async Task<DataSet> FetchDailyOutputSummaryAsync(IDbConnection connection, DateTime startDate, DateTime endDate, string area)
        {
            ...
            var results = await connection.QueryAsync<DailyOutputDto>(sql, new { StartDate = startDate.Date, EndDate = endDate.Date.AddDays(1).AddSeconds(-1), EndDatePlusOne = endDate.Date.AddDays(1), Area = area }, commandTimeout: 300);
